...
``aiida-rhino-zfs`` comes with a ``.github`` folder that contains continuous integration tests on every commit using `GitHub Actions <https://github.com/features/actions>`_. It will:
...
The documentation of ``aiida-rhino-zfs``
is ready for `ReadTheDocs <https://readthedocs.org/>`_:

Simply add the ``aiida-rhino-zfs`` repository on your RTD profile, preferably using ``aiida-rhino-zfs`` as the project name - that's it!
